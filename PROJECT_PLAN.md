...
**验收标准：**
- [ ] 所有接口已规格定义
- [ ] 插件链能按 Schema → Guard → Execute → Healing → Reflect 顺序执行
- [ ] 工具级插件配置优先于全局插件
- [ ] Hook事件在正确时机触发
- [ ] 单元测试覆盖率 ≥ 95%
- [ ] 代码通过 lint 和类型检查
...
- 单元测试：`tests/unit/test_tool_registry.py`
  ...
  - 工具配置验证
  - 插件链动态组合
- 集成测试：`tests/integration/test_tool_execution_flow.py`
  - 完整工具执行流程
  - 不同插件配置的工具

**验收标准：**
- [ ] Tool 类定义清晰，包含所有必要字段
- [ ] ToolRegistry 支持安全的工具管理
- [ ] 工具配置能正确映射到插件链
- [ ] 简单工具和复杂工具的插件配置能独立优化
- [ ] 单元测试覆盖率 ≥ 85%
...
| M2 - 核心引擎 | Phase 2 | 插件池和核心调度引擎可运行 | 2026-03-27 | | ⬜ |
| M3 - 插件体系 | Phase 3 | 5层基础插件实现完成 | 2026-03-31 | | ⬜ |
| M4 - 工具系统 | Phase 4 | 工具定义、注册、配置系统就绪 | 2026-04-02 | | ⬜ |
